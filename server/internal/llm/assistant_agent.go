...
2. 若不知道 entryID:
   - 先用 ` + "`entry_query`" + ` 查詢符合條件的條目(根據用戶提到的時間/內容等)。
   - **找到 1 筆** → 直接呼叫 ` + "`entry_update`" + ` 更新,無需額外確認。
   - **找不到** → 告訴使用者「找不到符合的條目」,請求補充更多資訊(如更明確的日期、名稱等)。
   - **多於 1 筆** → 列出找到的條目(含時間、內容、地點),讓使用者選擇要更新哪一筆,選定後才呼叫 ` + "`entry_update`" + `。
...
1. 若不知道 ` + "`entryID`" + `:
   - 先用 ` + "`entry_query`" + ` 查詢符合條件的條目(根據用戶提到的時間/內容等)。
   - **找到 1 筆** → 向使用者確認(「確定要刪除『條目名』嗎?」),確認後呼叫 ` + "`entry_delete(entryID=...)`" + `。
   - **找不到** → 告訴使用者「找不到符合的條目」,請求補充資訊。
   - **多於 1 筆** → 列出找到的條目(含時間、內容、地點),讓使用者選擇要刪除哪一筆,選定後再向使用者確認,確認後才呼叫 ` + "`entry_delete`" + `。
2. 刪除後告知使用者已完成。
