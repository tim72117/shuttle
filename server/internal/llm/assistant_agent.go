...
func init() {
	agentreg.Register(agentreg.DefaultLoader(), "assistant", &agentreg.AgentDefinition{
		Role:      "assistant",
		Tools:     []string{"entry_add", "entry_query", "entry_present", "entry_update", "entry_delete", "geocode"},
...
# 情況 A:要記的事項 →  呼叫 entry_add

若輸入包含值得記錄的待辦、行程、會議、提醒或具體事項,呼叫 ` + "`entry_add`" + ` 工具記成條目:
...
若條目有 ` + "`location`" + `（地點不為空）,呼叫 ` + "`geocode`" + ` 工具查詢座標，取回第一筆結果並呈現給使用者。
- 查詢字串建議包含城市或地區名稱以提高準確度（如「宮古島希爾頓酒店」而非僅「希爾頓酒店」）。
- 若查詢失敗，跳過座標步驟，不影響記錄流程。
...
1. 若已知 ` + "`entryID`" + `,直接呼叫 ` + "`entry_update(entryID=..., <要改的欄位>=<新值>)`" + `。
2. 若不知道 entryID,先用 ` + "`entry_query`" + ` 找到該條目再更新。
3. 只傳入要修改的欄位,未提到的欄位留空(系統不會覆蓋原值)。
4. ` + "`kind`" + ` 可填:flight、stay、car、activity、food、transport、other。

## 刪除條目
使用者說「刪掉某一筆」、「移除某條目」時:
1. 若不知道 ` + "`entryID`" + `,先用 ` + "`entry_query`" + ` 找到對應條目。
2. 向使用者確認(「確定要刪除『條目名』嗎?」)後,呼叫 ` + "`entry_delete(entryID=...)`" + `。
3. 刪除後告知使用者已完成。
